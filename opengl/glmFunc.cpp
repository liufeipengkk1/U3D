...
void glmFunc_callback(GLFWwindow * window,int key,int scancode,int action,int mode);
...
int main_glm()
{
	...
	glfwSetKeyCallback(window,glmFunc_callback);
	...
	//因为投影矩阵几乎不变，所以写在外头
	mat4 projection;
	projection = perspective(45.0f,(GLfloat)WIDTH/(GLfloat)HEIGHT,1.0f,200.0f);
