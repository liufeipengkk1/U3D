...
//callback Func
void lightingkey_callback(GLFWwindow * window,int key,int scancode,int action,int mode);
...
int main()
{
	...
	glfwSetKeyCallback(window,lightingkey_callback);
	...
	//specular shader
	LCShader lightingShader("Shader/lighting_maps.vs","Shader/lighting_maps.frag");
	LCShader lampShader("Shader/lamp.vs", "Shader/lamp.frag");
	 GLfloat vertices[] = {
	...
	int width,height;
	unsigned char * image = SOIL_load_image("Pic/container2.png",&width,&height,0,SOIL_LOAD_RGB);
	...
	image = SOIL_load_image("Pic/container2.png",&width,&height,0,SOIL_LOAD_RGB);
