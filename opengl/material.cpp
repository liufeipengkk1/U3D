...
int main()
{
	...
	//specular shader
	LCShader lightingShader("Shader/material.vs","Shader/material.frag");
	LCShader lampShader("Shader/lamp.vs", "Shader/lamp.frag");
	 GLfloat vertices[] = {
	...
	while(!glfwWindowShouldClose(window))
	{
        ...
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "light.ambient"),  ambientColor.x, ambientColor.y, ambientColor.z);
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "light.diffuse"),  diffuseColor.x, diffuseColor.y, diffuseColor.z);
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "light.specular"), 1.0f, 1.0f, 1.0f);
        // Set material properties
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "material.ambient"),   1.0f, 0.5f, 0.31f);
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "material.diffuse"),   1.0f, 0.5f, 0.31f);
        glUniform3f(glGetUniformLocation(lightingShader.shaderProgram, "material.specular"),  0.5f, 0.5f, 0.5f); // Specular doesn't have full effect on this object's material
        ...
        transform = glm::scale(transform, glm::vec3(0.5f)); // Make it a smaller cube
        glUniformMatrix4fv(tranlLoc, 1, GL_FALSE, glm::value_ptr(transform));
